...
const trustColor = (score) => {
  if (score >= 75) return { text: 'text-emerald-500 dark:text-emerald-400', bg: 'bg-emerald-500', glow: 'shadow-[0_0_20px_rgba(16,185,129,0.35)]', border: 'border-emerald-500/30' };
  if (score >= 45) return { text: 'text-amber-500 dark:text-amber-400',   bg: 'bg-amber-500',   glow: 'shadow-[0_0_20px_rgba(245,158,11,0.35)]', border: 'border-amber-500/30' };
  return               { text: 'text-rose-500 dark:text-rose-400',         bg: 'bg-rose-500',       glow: 'shadow-[0_0_20px_rgba(244,63,94,0.35)]', border: 'border-rose-500/30' };
};
...
export const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black transition-colors duration-500 text-slate-900 dark:text-white antialiased font-sans selection:bg-primary-500/30 selection:text-white pb-0 relative overflow-x-hidden">
      ...
      <motion.div
        ...
        className="absolute bottom-[20%] right-1/4 w-[500px] h-[500px] rounded-full pointer-events-none filter blur-[100px] z-0"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-6 mb-12 sm:mb-16">
          {statCards.map(({ label, value, Icon, color }, i) => (
             ...
             >
               ...
               <p className={`text-3xl sm:text-4xl font-display font-black tracking-tight ${color}`}>{value}</p>
             </motion.div>
          ))}
